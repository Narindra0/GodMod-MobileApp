...
import datetime
import os
from src.core.database import get_db_connection, PG_DATABASE, PG_HOST
...
def generate_sql_file(schem_info, output_path):
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        f.write(f"-- Audit du schéma PostgreSQL - {PG_DATABASE} @ {PG_HOST} - {timestamp}\n\n")
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        ...
        for table_name, info in schem_info.items():
            ...
            for col in info['columns']:
                f.write(f"| {col['name']} | {col['type']} | {'Oui' if col['not_null'] else 'Non'} | `{col['default_value']}` | {'Oui' if col['is_pk'] else 'Non'} |\n")
            f.write("\n")
            if info['constraints']:
                f.write("### Contraintes\n")
