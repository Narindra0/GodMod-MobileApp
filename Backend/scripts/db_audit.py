...
try:
    ...
    DEFAULT_DB_PATH = config.DB_NAME
except Exception:
    DEFAULT_DB_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "data", "godmod_database.db"))
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        ...
        for table_name, info in schem_info.items():
            ...
            if info['indexes']:
                ...
                f.write("\n")
    return filename
def generate_mermaid_diagram(schem_info, output_path, db_path: str):
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
...
if __name__ == "__main__":
    ...
    try:
        ...
        mermaid_file = generate_mermaid_diagram(schema_data, OUTPUT_DIR, DB_PATH)
