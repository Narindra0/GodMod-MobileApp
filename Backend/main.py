...
from src.core import database  # Import direct PostgreSQL
from src.analysis import intelligence
from src.analysis import ai_booster
...
def callback_predictions_ia(journee: int):
    ...
    try:
        ...
        bankroll = 20000 # Bankroll par defaut
        
        if selections_prisma:
            table_prisma = create_table(["Match", "Pred.", "Base", "IA Boost", "Final"], title=f"PRISMA x GEMINI - ANALYSE J{journee_prediction}")
            for p in selections_prisma:
                boost = p.get('boost_ia', 0.0)
                boost_str = f"[bold green]{boost:+.1f}[/]" if boost > 0 else f"[bold red]{boost:+.1f}[/]" if boost < 0 else "[dim]0.0[/]"
        ...
        if selections_zeus:
            ...
            with database.get_db_connection() as conn:
                ...
                row = cursor.fetchone()
                bankroll = row['bankroll_apres'] if row else 20000
...
def reset_database_api():
    ...
    try:
        ...
        logger.info(f"Base de données réinitialisée via API: {total_deleted} lignes supprimées, {equipes_count} équipes préservées")
