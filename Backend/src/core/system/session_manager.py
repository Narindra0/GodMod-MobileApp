...
def _create_new_session_internal(conn, previous_capital=None, active_session=...):
    ...
    cursor.execute(
        """
        INSERT INTO sessions (timestamp_debut, status, current_day, capital_initial, 
        type_session, score_zeus, score_prisma)
    ...
    return {"id": new_id, "current_day": 1, "capital_initial": capital_to_use, "score_prisma": prisma_to_use}
...
def _update_session_day_internal(conn, session_id, day_number):
    ...
    return {"id": session_id, "current_day": day_number}
