...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default='v2.0',
        help='Version du modèle (ex: v2.0)'
    )
    parser.add_argument(
        '--journee-debut-train',
        type=int,
        help="Première journée utilisée pour l'entraînement (mode manuel)"
    )
    parser.add_argument(
        '--journee-fin-train',
        type=int,
        help="Dernière journée utilisée pour l'entraînement (mode manuel)"
    )
    parser.add_argument(
        '--journee-debut-eval',
        type=int,
        help="Première journée utilisée pour l'évaluation (mode manuel)"
    )
    parser.add_argument(
        '--journee-fin-eval',
        type=int,
        help="Dernière journée utilisée pour l'évaluation (mode manuel)"
    )
    ...
    if args.journee_debut_train is not None:
        print(
            f"  - Train jours:     {args.journee_debut_train}"
            f" -> {args.journee_fin_train}"
        )
        print(
            f"  - Eval jours:      {args.journee_debut_eval}"
            f" -> {args.journee_fin_eval}"
        )
    print()
