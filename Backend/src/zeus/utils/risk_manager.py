        ...
        if montant_demande > max_possible:
            if max_possible < self.MISE_MIN:
                return (
                    False,
                    0,
                    f"Mise bloquée : Bankroll insuffisant ({self.capital_actuel}Ar) "
                    f"pour miser le minimum de {self.MISE_MIN}Ar.",
                )
            return (
                False,
                max_possible,
                f"Mise plafonnée : {montant_demande}Ar excède la limite de risque. "
                f"Maximum autorisé : {max_possible}Ar.",
            )
