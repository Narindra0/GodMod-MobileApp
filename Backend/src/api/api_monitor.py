...
def _get_journee_candidates(last_journee_db: int):
    api_journee = get_max_journee_from_api()
    if api_journee == 38:
        logger.info("[MONITOR] Exception J38 detectee sur l'API. Ignoration complete.")
        api_journee = None
    journee_cotes = None
    if api_journee is None:
        ...
        if last_journee_db != 1:
            logger.info(
                f"[MONITOR] Session neuve ou remise a zero (Resultats API vides). "
                f"Verification des cotes : J{journee_cotes}"
            )
    return api_journee, journee_cotes


def _handle_missing_api(consecutive_errors: int) -> int:
    consecutive_errors += 1
    wait_time = min(MONITOR_CONFIG["RETRY_DELAY"] * (2 ** (consecutive_errors - 1)), 300)
    logger.warning(
        f"[MONITOR] Impossible de recuperer journee API (Erreur {consecutive_errors}). "
...
def _should_start_new_cycle(journee_cotes: int | None, last_journee_db: int) -> bool:
    return journee_cotes == 1 and (last_journee_db >= 37 or last_journee_db == 0)


def _start_new_cycle(last_journee_db: int, callback_on_new_journee) -> int:
    logger.info("[MONITOR] Debut de cycle detecte (J1).")
    console.print()
    ...
    try:
        ...
        if matches_filtered:
            count = insert_api_matches(matches_filtered, session_id=session_id)
            print_success(f"{count} matchs inseres pour la nouvelle session {session_id}")
            last_journee_db = 1
    ...
    return last_journee_db


def _handle_new_journee(api_journee: int, last_journee_db: int, callback_on_new_journee) -> int:
    logger.info(f"[MONITOR] Nouvelle journee detectee : J{api_journee} (BDD: J{last_journee_db})")
    console.print()
    console.rule("[bold green]ALERTE : Nouvelle journee detectee ![/]")
