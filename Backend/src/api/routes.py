...
def register_routes(app: FastAPI) -> None:
    ...
    async def update_prisma_ml_settings(settings: PrismaSettingsUpdate):
        try:
            with get_db_connection() as conn:
                with conn.cursor() as cur:
    ...
    async def zeus_borrow(request: BorrowRequest):
        try:
            ...
            with get_db_connection(write=True) as conn:
                ...
                cursor.execute(
                    "SELECT bankroll_apres FROM historique_paris WHERE session_id = %s AND strategie = 'ZEUS' ORDER BY id_pari DESC LIMIT 1",
                    (session_id,),
                )
                row = cursor.fetchone()
    ...
    async def get_metrics_overview():
        try:
            ...
            with get_db_connection() as conn:
                ...
                profit_prisma = bankroll_prisma - config.DEFAULT_BANKROLL
                total_profit = profit_zeus + profit_prisma

                cursor.execute(
                    "SELECT score_prisma, score_zeus, dette_zeus, total_emprunte_zeus, stop_loss_override FROM sessions WHERE id = %s",
                    (session_id,),
                )
