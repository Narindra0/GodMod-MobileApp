...
def _predict_with_blending(results: Dict, available_models: List[str]) -> dict:
    ...
    try:
        from prisma.models import xgboost_model, catboost_model, lightgbm_model
    except ImportError:
        sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))
        from prisma.models import xgboost_model, catboost_model, lightgbm_model
...
def train_ensemble(conn, force=False, train_meta_learner: bool = True):
    ...
    try:
        from prisma.training.training_triggers import should_retrain_models, get_training_summary
    except ImportError:
        sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))
        from prisma.training.training_triggers import should_retrain_models, get_training_summary
