...
def load_all_data():
    ...
    with get_db_connection() as conn:
        df_session = pd.read_sql_query(
            "SELECT score_prisma as score, score_zeus FROM sessions WHERE id = ?", conn, params=(session_id,)
        )
        df_total = pd.read_sql_query(
            "SELECT COUNT(*) as total FROM predictions WHERE session_id = ? AND succes IS NOT NULL",
            conn,
            params=(session_id,),
        )
        df_wins = pd.read_sql_query(
            "SELECT COUNT(*) as wins FROM predictions WHERE session_id = ? AND succes = 1", conn, params=(session_id,)
        )
        df_preds = pd.read_sql_query(
            """
            SELECT m.journee as J, e1.nom as Domicile, e2.nom as Exterieur, 
            p.prediction as Prono, p.resultat as Reel, p.succes
            FROM predictions p
        ...
        df_trend = pd.read_sql_query(
            f"SELECT id, (CASE WHEN succes = 1 THEN {config.PRISMA_POINTS_VICTOIRE} "
            f"ELSE {config.PRISMA_POINTS_DEFAITE} END) as points_gagnes "
            f"FROM predictions WHERE session_id = ? AND succes IS NOT NULL ORDER BY id",
            conn,
            params=(session_id,),
        )
...
with m1:
    st.metric("Score Global", f"{score} pts")
with m2:
    st.metric("Taux de Réussite", f"{win_rate:.1f}%")
with m3:
    st.metric("Total Prédictions", f"{total_history}")
with m4:
...
with col_left:
    ...
    with tab_results:
        ...
        if not df_results.empty:
            ...
            journee_selectionnee = st.selectbox("📅 Sélectionner la journée", journees, index=0, key="journee_selector")
...
if current_intelligence_state:
    st.sidebar.markdown(
        """
        <div style='background-color: rgba(35, 134, 54, 0.2); padding: 10px; 
        border-radius: 5px; border-left: 3px solid #238636;'>
    ...
    st.sidebar.markdown(
        """
        <div style='background-color: rgba(248, 81, 73, 0.1); padding: 10px; 
        border-radius: 5px; border-left: 3px solid #f85149;'>
